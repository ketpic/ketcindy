...
\item[機能]KeTJSファイルのhtmlの前後ににスクリプトを追加したhtmlを作成する。
...
【例】Setedittext(0,["y=","Size=18","Width=100"]); Text0の表示文字\verb|"y="|，文字サイズ18pt，幅100pt
...
\vspace{\baselineskip}
【ファイル操作】\\
\hyperlink{isexists}{isexists}\>dirにfileが存在するか\\
\hyperlink{makedir}{makedir}\>pathにdirnameのdirを作成する\\
\hyperlink{filelist}{Filelist}\>(mainpathにある)dirnameのファイル一覧をリストとして返す\\
\hyperlink{readlines}{Readlines}\>(pathにある)filernameの各行を読み込みリストとして返す\\\hyperlink{gethome}{gethome()}\>ユーザホームを返す\\
...
\hyperlink{bracket}{Bracket(str)}\>strから bracketpair を検索し位置と階層レベルのリストを返す\\\hyperlink{getlevel}{Getlevel(str)}\>strから sepの位置と階層レベルのリストを返す\\
...
\vspace{\baselineskip}
【Wolfram Engineとの連携】\\
\hyperlink{calcbyW}{CalcbyW(name,list,option)}\>Wolfram 言語のスクリプトを実行する\\\hyperlink{wlfun}{Wlfun(name,式,list,option)}\>Wolfram 言語の1つの関数を実行する\\
...
\hyperlink{ketcindyjmain}{Ketcindyjsbody(prependlist,appendlist)}\>htmlの前後にスクリプトを追加したhtmlを作成\\
