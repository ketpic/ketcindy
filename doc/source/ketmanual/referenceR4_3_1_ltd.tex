...
\item \verb|\|（バックスラッシュ）を出力するには２つ並べてかく．
...
%\tab{Mix(D1, D2, $\cdots$)}FD列，MS，MLから（１段上の）MSまたはMLを作成
...
%\tab{MixL(D1,D2, $\cdots$)}MS, MLから（１段上の）MLを作成
...
%\tab{Mixtype(Data)}Dataが基本データなら１，MSなら２，MLなら３を返す
...
\hspace*{16zw}\tab[6zw]{"E=関数"}関数の０点は除外\\
...
\tab{Openfile("ファイル名"\{, "単位長", "SF=ソースファイル名"\}) }\\
\tab{}書き出し用ファイルを開く（デフォルトは画面）\\
\rei cd('C:/TeXF/'); \\
\reicr Openfile(\,'fig.tex'\,);\\
\reicr Openfile(\,'fig.tex',\,'fig.r'\,);\\
\chuu 単位長を指定すると Beginpicture("単位長")まで書き出す\\
\chuu ソースが同一フォルダにあるときは，SFの指定は不要
...
\tab{Closefile(\{"1"または "0"\})}書き出し用ファイルを閉じる（デフォルト＝画面に戻す）\\
\chuu "1"または"0"の文字列を指定\\
\hspace{20zw}Endpicture(1または0)を書き出す
...
\rei Htickmark("mn")（目盛りと数を１間隔でつける）\\
\rei Vtickmark("m1n2r1.5")（1.5倍した数を１つとびに）

\tab[13zw]{Shade(\,PD列またはlist\{, 濃さ\}\,)\mbox{}}\\
\tab{}閉曲線の内部を塗りつぶし　濃さ: 0 〜 1（デフォルトは１）
...
\tab{Bowname(弓形，数式\{,\ 方向\} )}\\
\tab{}弓形PDの「方向」に式を書き入れる\\
\chuu 方向のデフォルトは　"c"\\
\rei Gb=Bowdata( A, B, 1, 0.5);\\
\reicr Bowname(Gb, 'd');

\tab{Bownamerot( 弓形\{, 接線方向\{, 法線方向\}\}, 数式\{，向き\} )}\\
...
\hspace*{180pt}PDが閉曲線のときは上の２つのPDをつなげる．
...
%\tab{}曲面データリストFDの３次元ワイヤフレームデータを生成\\
%\chuu デフォルトは　点の個数＝c(50,\ 50)，分割数$<$- 25\\
%\chuu 点の個数を数nで指定したときは２方向ともnになる\\
%\rei Out$<$- Sf3data(Fd)

%\tab{Sfparadata(〃)}FDのワイヤフレームデータを平行投影した２次元データ

%\tab{Sfpersdata(〃)}FDのワイヤフレームデータを平行投影した２次元データ
...
%\noindent
%\chuu 曲面上の３次元データとして得られる．\\
%\chuu BorderHiddenData()で
%陰線の３次元データが得られる．\\
%\chuu CuspData()で輪郭線の３次元データが
%得られる．\\
%\chuu CuspPt()でcusp点の２次元データが
%得られる．\\
%\chuu BorderPt()で輪郭線の交点の２次元データが
%得られる．
...
%\chuu CrvonsfHiddenData()で陰線の３次元データが得られる．
...
%\chuu Crv3onsfHiddenData()で陰線の３次元データが得られる．
...
%\chuu CrvsfHiddenData()で陰線の３次元データが得られる．\\
...
\hspace*{18zw}"Cna=論理値"　１行を列名にするか（デフォルトTRUE） \\
\hspace*{18zw}"Rna=論理値"　１列を行名にするか（デフォルトFALSE） \\
\rei DL$<$- Readtextdata("dt.csv",\ c(2,\ 1),\ "R=1000",\ "C=2")

\tab{Writetextdata(データフレーム，ファイル名)}\\
\tab{}データフレームを .csvファイルに書き出す\\
\chuu 列名は１行目におき，NAはblankにする\\
