...
This function have to write first line on Draw slot page in case of plane figure and Initialization slot page in case of space figure (KeTCindy's 3D-mode).
\end{cmd}
...
\begin{center}
\includegraphics[bb=0 0 227 205 , width=4cm]{Fig/setorigin.pdf}\hspace{10mm}%
\input{Fig/setorigin}
\end{center}
...
\itemket{Description}Generic function to make a point data.
\itemket{Examples}\mbox{}
...
\verb|Pointdata("4",[A,B],[0]);      |  // white circles\\
...
%------------Arrowdata--------------------------------
\begin{cmd}{Arrowdata}{arrow data}
...
Options : arrowhead size, arrowhead angle,position,line type,line color.\\
All options do not always reflect on Euclidean view.
\itemket{Examples}\mbox{}

\verb|Arrowdata([A,B]);|\\
\verb|Arrowdata("1",[[2,0],[4,3]],[1,45,0.5]);  | //(2,0) to (4,3) , angle : 45$^\circ$\\
...
\itemket{Usage}Letter([position, direction, string])
...
\hspace{15mm} draw text \verb|"Expr=n,str"|
...
The location of expressions can be modified via \verb|"Expr=tn, expressions"| where 
...
%------Drawsegmark--------------------------------------
\begin{cmd}{Drawsegmark}{drawsegmark}
\itemket{Usage}Drawsegmark(name, list, options) or Segmark(name, list, options)
\itemket{Description}Add a mark to a segment. 
\itemket{Details}
Add a mark to the segment determined by the end points specified in the list. 
Four kinds of marks can be used.\\
Extra options :\\
\verb|"Type=n"| (n=1,2,3,4) specifies the kind of mark.\\
\verb|"Width="| specifies the distance between two segments of the mark 
(in case when $n=2$).
\itemket{Examples}\mbox{}

\verb|Listplot([A,B,C,D,A]);|\\
\verb|Segmark("1",[A,B],["Type=1"]);|\\
\verb|Segmark("2",[B,C],["Type=2","Width=1.5"]);| //width of two lines\\
\verb|Segmark("3",[C,D],["Type=3"]);|\\
\verb|Segmark("4",[D,A],["Type=4"]);|
\begin{center}
\input{Fig/segmark}
\end{center}
\end{cmd}
...
%--------------Rulerscale------------------------------
\begin{cmd}{Rulerscale}{ruler-scale}
...
\verb|["r",a,b,c]|  to put marks from \verb|a| to \verb|b| with intervals \verb|c|.
...
\itemket{Details}Added options are angle(degree) and interval(ratio) of hatches.
\itemket{Examples}\mbox{}
...
\itemket{Usage}Shade(list of PD, options);
\itemket{Description}Generic functio to fill a domain surrounded by a closed curve.
\itemket{Details}The options are color and density.
\itemket{Examples}\mbox{}

\verb|Setax([7,"nw"]);|\\
\verb|Plotdata("1","2*sin(x)","x",["Num=100"]);|\\
\verb|Lineplot("1",[[0,1],[1,1]]);|\\
\verb|Enclosing("1",["ln1","Invert(gr1)"],[[2,1],"nodisp"]);|\\
\verb|Shade(["en1"],["Color=[0,0,1]",0.5]);|

\begin{center}
\input{Fig/shadeE}
\end{center}

\verb|Plotdata("1","x^2-1","x");|\\
\verb|Plotdata("2","x^2/2","x");|\\
\verb|Enclosing("1",["gr1","Invert(gr2)"],[[-1,1],"nodisp"]);|\\
\verb|Shade(["en1"],["Color=red"]);|
...
\verb|Listplot("2",[A+[r2,0],A+[r1,0]],["nodisp"]);|\\
\verb|Joincrvs("1",["cr1","sg1","Invert(cr2)","sg2"],["nodisp"]);|\\
\verb|Shade(["join1"],["Color=green"]);|
...
\verb|    cmdL=[ "=rnorm",[10] ];|\\
\verb|    CalcbyR("dt",cmdL);|

calculate the sample mean and the sample standard deviation of the data.\\
\verb|    nx=length(dt);|\\
\verb|    mx=sum(dt)/nx;|\\
\verb|    sx=sqrt(dt*dt/nx-mx^2);|

output the data, the mean and the SD to the console.\\
\verb|    println("Data : "+dt);|\\
\verb|    println("Mean : "+format(mx,4));|\\
\verb|    println("SD : "+format(sx,4));|\\

\begin{center}
\includegraphics[bb=0.00 0.00 532.53 249.51,width=12cm]{Fig/calcbyr01.pdf} 
\end{center}

generate 10 random samples from the normal distribution $N(50,5^2)$  and calculate the sample mean and the unbiased variance of the data.\\
\verb|    cmdL=[|\\
\verb|      "tmp1=rnorm",[10,50,5],|\\
\verb|      "tmp2=mean",["tmp1"],|\\
\verb|      "tmp3=var",["tmp1"],|\\
\verb|      "=c(tmp1,tmp2,tmp3)",[]|\\
\verb|    ];|\\
\verb|    CalcbyR("rd",cmdL);|\\
\verb|    dt=rd_(1..(length(rd)-2));|\\
\verb|    mx=rd_(-2);|\\
\verb|    vx=rd_(-1);|\\
\verb|    println("data : "+dt);|\\
\verb|    println("Mean : "+format(mx,4)+" variance : "+format(vx,4));|
...
\itemket{Usage}Scatterplot(name,filename,option)
\itemket{Description}This command draw scatter plot reading a csv file.
\itemket{Details}Options are:\\
\verb|"Reg=yes(no:default)"| to decide whether to draw the regression line or not.\\
\verb|"Size=number(1:defaut)"| to decide the size of points.\\
\itemket{Examples}\mbox{}

\verb| Scatterplot("1","data.csv",["Reg=yes","Size=5"]);|\\
...
\begin{layer}{150}{0}
\putnotese{70}{0}{ \input{Fig/maxfun01}}
\end{layer}
...
\begin{layer}{150}{0}
\putnotese{70}{20}{ \input{Fig/maxfun02}}
\end{layer}
...
\subsection{Chart of Making Slides}

\hspace{10mm}\input{Fig/makeslide}

\hspace{10mm}makeslide.cdy

\hspace{10mm}\includegraphics[bb=0.00 0.00 995.05 436.02,width=10cm]{Fig/slide01E.pdf} 


\subsection{Contents of Text File}
...
\begin{layer}{150}{0}
\putnotese{80}{10}{\includegraphics[bb=0.00 0.00 582.03 386.02,width=7cm]{Fig/sliderepeatE.pdf} }
\end{layer}
\begin{verbatim}
...
The main area which is on the left side of the screen and whose NE and SW are diagonal shows the range output to the \TeX document. The viewpoint can be moved with the slider under the main area, and the axis on the main area rotates.

In the subarea, the viewpoint is fixed on the xy plane.
...
When adding points to the main area with the drawing tool of Cinderella, we will recognize them as points in space by the internal function \verb|Ptseg3data| and draw points corresponding to the subarea.
However, initially, the z coordinate is 0.

For example, if the name of the point is "A'', the point on the sub area is displayed as "Az''. 
For the point "A'', "(A. x, A. y)'' is the coordinates in the main area and the corresponding point in 3D-space has the coordinate of "A3d''.
If you change the name of the point "A'' with the inspector to "D'', you will add a point with the name corresponding to the subarea, but be careful as the previous point "Az'' will not disappear.The remaining points can be erased with the delete button \includegraphics[bb=0 0 6.48 5.04 , width=0.6cm]{Fig/delete.pdf} of the Cinderella.
...
\itemket{Description}Creates subarea, and recognize 3D points.
\itemket{Details}Creates subarea, and treat geometric points as 3D points in that area. 3D points is not output in the \TeX. \\
\textcolor{red}{Caution}
This funcition has to write at the beginning of Draw slot.\\
Option is the list of exclusion points. Exclusion points are independent of view point. 
% sample cdy files are s0521totaldiffskelton.cdy and s0522conecutmaxima.cdy
%\itemket{Examples} \mbox{}
\end{cmd}
...
makes [[A,B,C,D],[[1,2,3],[1,2,4],[1,3,4],[2,3,4]]]
This data is used to drawing tetrahedron.
For example code, see \hyperlink{vertexedgeface}{VertexEdgeFace()}.
\end{cmd}
...
For example code, see \hyperlink{execcmdc}{ExeccmdC()}.
\end{cmd}
...
\itemket{Description}Generic function to draw 3D-surface. 
\itemket{Details}options1=no option or " "(space) or "r" or "m" and "Wait=integer". Default value of Wait is 20.\\
...
option2="nodisp" or line type. Default is "nodisp".\\
...
\verb|Putpoint3d(["A",[3,3,3],"B",[0,1,0],"C",[0,0,1]],"fix");|\\
...
\verb|Putpoint3d(["A",[0,-3,0],"B",[0,3,2]],"fix");|\\
\verb|Spaceline("1",[A,B]);|\\
\verb|fd=[|\\
...
\itemket{Details}Specify a line segment with two endpoints. Specify the plane as three points that it contains. Options are ``put'' or ``draw'', and if omitted select ``draw''.\\
...
\verb|  Hn=3; |\\
\verb|  Putaxes3d(Hn); |\\
\verb|  Putpoint3d("A",[Hn,Hn,0],"fix"); |\\
\verb|  Putpoint3d("B",[Hn,0,Hn],"fix"); |\\
\verb|  Putpoint3d("C",[Hn,Hn,Hn],"fix"); |\\
\verb|  Putpoint3d("D",[0,Hn,Hn],"fix"); |\\
...
\verb|  VertexEdgeFace("1",phd,["Edg=nogeo"]); |\\
...
\verb|  Putpoint3d(["O",[0,0,0]],"fix"); |\\
\verb|  Putpoint3d(["P",[1,1,1]],"fix"); |\\
\verb|  Perpplane("E-F","P",P3d-O3d,"put"); |\\
\verb|  vec1=2*(E3d-P3d); |\\
\verb|  vec2=2*(F3d-P3d); |\\
\verb|  Putpoint3d(["A",P3d+vec1+vec2],"fix"); |\\
...
\verb|Xyzax3data("","x=[-5,5]","y=[-5,5]","z=[-5,4]"); |\\
\verb|Putpoint3d("O",[0,0,0],"fix"); |\\
\verb|Putpoint3d("A",[3,0,0],"fix"); |\\
\verb|Putpoint3d("B",[0,3,0],"fix"); |\\
\verb|Putpoint3d("C",[0,0,3],"fix"); |\\
\verb|Perppt("H","O","A-B-C","put"); |\\
...
\itemket{Details}Name2 is the plotting data of polyhedron which we get form the function VertexEdgeFace().
\itemket{Examples}  \mbox{}
...
\verb|VertexEdgeFace("s06",phd,["Edg=nogeo"]);|\\
...
\verb|Phparadata("1","1");|
\begin{center}
 \includegraphics[bb=0 0 1452 730 , width=10cm]{Fig/phparadata01.pdf}
\end{center}
...
\itemket{Details}Option is "fix" or "free"(default).
\itemket{Examples}  \mbox{}

\verb|Putpoint3d(["A",[2,1,3]]);|\\
\verb|Putpoint3d(["A",[2,1,3]],"free");|  //same as above\\
...
\itemket{Details}Option is ["size=n"] then we get the magnification of n times. If n is negative value then we have the image of vertical inversion.
\itemket{Examples}  \mbox{}

\verb|Setdirectory( Dirhead+"/data/polyhedrons_obj");|  //Many polyhedron data exist in this directory.\\
\verb|polydt=Readobj("r02.obj",["size=2"]);|  //"r02" is the name of regular octahedron data.\\
\verb|Setdirectory(Dirwork);|  //Chage work space.\\
\verb|VertexEdgeFace("1",polydt,["Edg=nogeo"]);|  //output data name is phf3d1\\
\verb|Nohiddenbyfaces("1","phf3d1");|
...
\verb|Putpoint3d(["A",[0,-2,0],"B",[2,-2,0],"C",[1,-2,2],"D",[1,-2,3]],"fix"); |\\
\verb|Spaceline("1",[A,B,C,A]); |\\
...
\verb|Putpoint3d(["A",[0,-2,0],"B",[2,-2,0],"C",[1,-2,2]],"fix");|\\
\verb|Spaceline("1",[A,B,C,A]);|\\
...
\verb|Putpoint3d(["A",[2,-1,2]],"fix");|\\
\verb|pt=Scalepoint3d(A3d,[3,2,4],[1,1,1]); //pt=[4,-3,5]|\\
\verb|Putpoint3d(["B",pt],"fix");|
...
\itemket{Usage}Sfbdparadata(name, list, list of options1, list of options2)
\itemket{Description}Generic function to draw the surface by performing hidden line processing.
...
\verb|fd=["x=x^2-y^2","x=[-2,2]","y=[-2,2]"];|\\
\verb|if(Isangle(),|  //selecting the slider point, draw wire frame surfaces.\\
\verb|  Sf3data("1",fd);|\\
\verb|  ,|\\
\verb|  Startsurf();|\\
\verb|  Sfbdparadata("1",fd);|\\
\verb|  ExeccmdC("1");|  //draw the surface\\
\verb|);|\\
(left figure)\\

\verb|Sfbdparadata("1",fd,["dr,2"],["do"]);|  //the surface with thick line, hidden line with dotted line.\\
(right figure)
\begin{center} \input{Fig/sfbdpara01}    \input{Fig/sfbdpara02}\end{center}

\verb|fd=["z=4-(x^2+y^2)","x=R*cos(T)","y=R*sin(T)","R=[0,2]","T=[0,2*pi]","e"];|\\
\ \ //"e"=east indicate the boundary line $R=2$, $0 < T < 2\pi$.\\
\verb|Sfbdparadata("1",fd,[],["da"]);|
...
\verb|  Putpoint3d(["A",[0,-2,-2]],"fix");|\\
\verb|  Putpoint3d(["B",[-1,1,3]],"fix");|\\
...
\verb| pt2d=apply(pt,[Projcoordpara(#)_1,Projcoordpara(#)_2]); |\\
\verb| Spaceline("1",pt); |\\
\verb| Ptsize(3); |\\
\verb| Drawpoint(pt2d);|
...
Option is the following.\\
no option: output edges as the geometric elements but don't to \TeX file.\\
\verb|["Edg=nogeo"]|: output data to \TeX file.\\
\verb|["Pt=free"]|: Vertexes are free geometric points.
\itemket{Examples}  \mbox{}

\verb| Putpoint3d("A",2*[-1,-1/sqrt(3),0],"fix"); |\\
\verb| Putpoint3d("B",2*[1,-1/sqrt(3),0],"fix"); |\\
\verb| Putpoint3d("C",2*[0,sqrt(3)-1/sqrt(3),0],"fix"); |\\
\verb| Putpoint3d("D",2*[0,0,sqrt(3)],"fix"); |\\
...
\itemket{Usage} Wireparadata(name, PD, formula, integer, integer, options1, options2)
...
options1=no option or " "(space) or "r" or "m" and "Wait=integer". Default value of Wait is 30.\\
No option or " "(space) means\\
(1) If there exist no deta then it make a new data file.\\
(2) If there exist deta then it read the data file.\\
"m" means that it remake the new data file.\\
...
option2="nodisp" or line type of hidden line. Default is "nodisp", in this case we denote that option1 is empty letter list:\verb|[""]|.
\itemket{Examples}  \mbox{}

\begin{layer}{150}{0}
\putnotese{95}{5}{ \input{Fig/wirepara1}}
\end{layer}
...
\begin{layer}{150}{0}
\putnotese{95}{10}{ \input{Fig/wirepara2}}
\end{layer}
...
\begin{layer}{150}{0}
\putnotese{95}{10}{ \input{Fig/wirepara3}}
\end{layer}
