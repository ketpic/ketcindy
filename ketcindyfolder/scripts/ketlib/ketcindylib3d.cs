...
println("ketcindylib3d(2017.06.02) loaded");
...
Bezier3(nm,ptlistorg,ctrlistorg,options):=(
  regional(name,name3,name2,tmp,tmp1,tmp2,
      Ltype,Noflg,opstr,eqL,ii,knt,ctr,out);
  name="bz"+nm;
  name3="bz3d"+nm;
  name2="bz2d"+nm;
  tmp=Divoptions(options);
  ...
  eqL=tmp_5;
  opcindy=tmp_(length(tmp));
  tmp1=select(eqL,indexof(#,"Num")>0);
  if(length(tmp1)==0,eqL=append(eqL,"Num=20"));
  ptlist=ptlistorg;
  ...
  forall(1..(length(ptlist)-1),ii,
    ...
    Spacecurve(name+text(ii),tmp,"T=[0,1]",append(eqL,"nodisp"));//17.01.04
...
Putbezier3data(name,pt3Lorg,options):=(
...
//    if(nn==2,numstr="",numstr=text(nn-2));
...
Mkbezierptcrv3(ptdata,options):=(
  regional(Eps,ptlist,nn,pt3L,name,bz,pt,p1,p2,p13d,p23d,pt3,
    Out,tt,pos,tmp,tmp1,tmp2);
  Eps=10^(-4);
  pos=[NE.x+1,NE.y];
  if(MeasureDepth(ptdata)==0,ptlist=[ptdata],ptlist=[ptdata]);
  Out=[];
  forall(1..length(ptlist),nn,
    nn=1;
    pt3L=ptlist_nn;
	...
	tmp2=Putbezier3data(name,pt3L,options);
    Bezier3(name,pt3L,tmp2,options);
//    BZLIST3=append(BZLIST3,[name,pt3L]);
    Out=append(Out,tmp2);
...
//help:Phparadata("1","1",["do"]);
  ...
  if(Noflg<3,
    if(Outflg>=1,
      ...
      GLIST=append(GLIST,tmp);
      if(Outflg==1,
        SCIWRLIST=append(SCIWRLIST,name3);
        SCIWRLIST=append(SCIWRLIST,name2);
        SCIWRLIST=append(SCIWRLIST,nameh3);
        SCIWRLIST=append(SCIWRLIST,nameh2);
      );
    );
    if(Inflg==1,
      ...
      tmp=parse(name2);
      if(islist(tmp),
        Extractdata(name2,["nodisp"]);  // 12.19
      );
      SCIRELIST=append(SCIRELIST,name3);
      SCIRELIST=append(SCIRELIST,name2);
    );
  );
  if(Noflg<2,
    if(isstring(Ltype),
