...
//      wait(WaitUnit);
  ...
  ,
//    tmp=load(file);//16.10.23from
//	println([1449,tmp]);
//    tmp=replace(tmp,"/","");
//    tmp=tokenize(tmp,",");
//    tmp=name+"="+textformat(tmp,5);
//    parse(tmp);//16.10.23upto
    bp=parse(name);
    pstr="[";
    tmp1=[bp_1,ypos-dy/2];
    tmp2=[bp_1,ypos+dy/2];
...
MkprecommandR():=MkprecommandR(5);  // 15.02.01
MkprecommandR(prec):=(
  ...
  forall(1..length(Plist),
    ...
    cmdL=concat(cmdL,
         ["Assign("+Dq+Pnamelist_#+Dq+","+Pvaluelist_#+")",[]]);
  );
  ...
  forall(VLIST,
    ...
    if(!isstring(tmp1), 
      if(islist(tmp1),
        tmp2="[";
        forall(tmp1,
          if(abs(#)<10^(-5),  // 16.01.30
            tmp2=tmp2+"0,";
          ,
            tmp2=tmp2+format(#,prec)+",";
          );
        );
        tmp1=substring(tmp2,0
		,length(tmp2)-1)+"]";
      ,
        tmp1=format(tmp1,prec);
      );
    );
    cmdL=concat(cmdL,["eval(parse(text="+tmp+Dq+tmp1+Dq+")",[]]);
	tmp=substring(tmp,0,length(tmp)-1);
    cmdL=concat(cmdL,
        ["Assign("+Dq+tmp+Dq+","+tmp1+")",[]]); // 15.01.27
  );
  forall(OutFileList,
    cmdL=concat(cmdL,["tmp=ReadOutData",[Dq+#+Dq]]);
  );
//  forall(FUNLIST,  // 16.04.23from
//    cmdL=concat(cmdL,[#,[]]);
//  );  // 16.02.17 upto
//  forall(GLIST,
//    tmp1=Rform(#); 
...
//help:Implicitplot(options2=["Wait=10","Out=no","Mx=no"]);
  ...
  tmp=apply([fun,varx,vary],Dq+#+Dq);
  tmp_1=Sciform(tmp_1);
  tmp=concat(tmp,reL);
//  tmp=append(tmp,"Error"); // 16.02.29
  cmdL=MkprecommandS();
  cmdL=concat(cmdL,[
    "evstr",[tmp1,"Error"], // 16.03.15
    name+"=Implicitplot",tmp,  // 16.03.15
    "WriteOutData",[Dq+fname+Dq,Dq+name+Dq,name]
  ]);
//  tmp=[""];
  ErrFlag=0;   // 16.02.29 from
  if(mxcheck==1, 
    Mxfun(name,"expand",[fun],["Disp=n"]);
    tmp=parse("mx"+name);
    if(!isstring(tmp),ErrFlag=1);
  );  // 16.02.29 upto
  options=concat(options,["Wait="+text(waiting)]);
  if(wflg==1,options=append(options,"m"));
  if(wflg==-1,options=append(options,"r"));
  if(ErrFlag==0,
    CalcbyS(name,cmdL,options);
  );
  if(ErrFlag==1,
...
Hatchdata2(nm,iostr,pltlist):=Hatchdata2(nm,iostr,pltlist,[]);
Hatchdata2(nm,iostr,pltlist,options):=(
//  help: Hatchdata2("1",["i"],[["crAB"]],["dr,0.7"]);
  regional(name,Ltype,Noflg,PdL,tmp,tmp1,tmp2,opstr,opcindy,Str,Outflg,Inflg);
  name="ha"+nm;
//  println("output   hatchdata2 "+name);
  tmp=Divoptions(options);
  Ltype=tmp_1;
  Noflg=tmp_2;
  Inflg=tmp_3;
  ...
  if(islist(iostr),
    ...
    Str=Str+substring(tmp,0,length(tmp)-1)+"),";
  ,
    Str=Str+Dq+iostr+Dq+",";
  );
  PdL=pltlist;
  if(isstring(PdL_1) % !islist(PdL),PdL=[PdL]);
  forall(PdL,tmp1,
    tmp="list(";
    forall(tmp1,
      if(length(#)>1,
  ...
  Str=substring(Str,0,length(Str)-1)+opstr+")";
  if(Noflg<3,  // 2015.07.16
      GLIST=append(GLIST,name+"="+Str);
  );
  if(Noflg<2, 
    if(Outflg==1,
      println("Output Hatchdata2 "+name);
...
Hatchdata(nm,iostr,pltlist,optionorg):=(
...
// old          tmp1=tokenize(tmp,",//,");
// old         tmp1=apply(tmp1,if(isstring(#),Dq+#+Dq,textformat(#,dig)));
          num="1234567890+-.";
          tmp1=tokenize(","+tmp,",//");
...
ReadCdata(var, fname,options):=(
  ...
  GLIST=Append(GLIST,"Tmpstr=ReadOutData("+Dq+file+Dq+")");//17.06.16from
  GLIST=Append(GLIST,"execstr(Tmpstr)");
