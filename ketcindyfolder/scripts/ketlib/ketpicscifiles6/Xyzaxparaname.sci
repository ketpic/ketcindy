...
function Kekka=Xyzaxparaname(varargin)
  global MilliIn;
  ...
  Ph=Parapt(Px); Qh=Parapt(Qx); R=Vecnagasa(Ph,Qh);
  Kekka=[];
  if R>Eps
    Ch=Qh+Dr/R*(Qh-Ph);
  ...
  Ph=Parapt(Py); Qh=Parapt(Qy); R=Vecnagasa(Ph,Qh);
  if R>Eps
    Ch=Qh+Dr/R*(Qh-Ph);
    Kekka=Mixadd(Kekka,Pointdata(Ch));
    Expr(Ch,'c',Yname);
  end;
  Ph=Parapt(Pz); Qh=Parapt(Qz); R=Vecnagasa(Ph,Qh);
  if R>Eps
    Ch=Qh+Dr/R*(Qh-Ph);
    Kekka=Mixadd(Kekka,Pointdata(Ch));
    Expr(Ch,'c',Zname);
