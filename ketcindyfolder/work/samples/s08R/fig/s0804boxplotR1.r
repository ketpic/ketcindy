...
if(is.list(R1)){##
  for(ii in Looprange(1,length(R1))){##
    if(is.list(R1[[ii]])){##
      cat('[',file='s0804boxplotR1.txt',sep='',append=TRUE)##
      cat(sharps,file='s0804boxplotR1.txt',sep='',append=TRUE)##
      for(jj in Looprange(1,length(R1[[ii]]))){##
        cat(R1[[ii]][[jj]],file='s0804boxplotR1.txt',sep=',',append=TRUE)##
        cat(sharps,file='s0804boxplotR1.txt',append=TRUE)##
      }##
      cat(']',file='s0804boxplotR1.txt',sep=',',append=TRUE)##
      cat(sharps,file='s0804boxplotR1.txt',append=TRUE)##
    }else{##
      cat(R1[[ii]],file='s0804boxplotR1.txt',sep=',',append=TRUE)##
      cat(sharps,file='s0804boxplotR1.txt',append=TRUE)##
    }##
  }##
}else{##
  cat(R1,file='s0804boxplotR1.txt',sep=',')##
  cat(sharps,file='s0804boxplotR1.txt',append=TRUE)##
}##
cat('////',file='s0804boxplotR1.txt',sep=',',append=TRUE)##
quit()##
