...
function FsL=Wirepersdata(varargin)
  ...
  for I=1:length(DuL)
    ...
    FkL=Cuspsplitpers(Tmp,Fd,Eps(1));  //
    for J=1:Mixlength(FkL)
      ...
      Fs=Nohiddenpers2(Par,Fk,Fd,1,Np,Eps);
  ...
  for I=1:length(DvL)
    V=DvL(I);
    Tmp=Paramplot('[U,V]','U=[Umin,Umax]','N=Np(1)');
    FkL=Cuspsplitpers(Tmp,Fd,Eps(1));  //
    for J=1:Mixlength(FkL)
      Fk=Mixop(J,FkL);
      Tmp=Projpers(Fk);
      Par=Partitionseg(Tmp,Fbdy,Eps(1),Eps2);  //17.05.07
      if Mixlength(PARTITIONPT)>2
        for K=2:Mixlength(PARTITIONPT)
          Tmp1=Mixop(K,PARTITIONPT);
          WIREPT=Mixadd(WIREPT,Tmp1);
